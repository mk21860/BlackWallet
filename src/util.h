...
template<typename T>
std::string HeRETr(const T itbegin, const T itend, bool fSpaces=false)
{
    std::string rv;
...
inline std::string HeRETr(const std::vector<unsigned char>& vch, bool fSpaces=false)
{
    return HeRETr(vch.begin(), vch.end(), fSpaces);
}

template<typename T>
void PrintHex(const T pbegin, const T pend, const char* pszFormat="%s", bool fSpaces=true)
{
    printf(pszFormat, HeRETr(pbegin, pend, fSpaces).c_str());
}

inline void PrintHex(const std::vector<unsigned char>& vch, const char* pszFormat="%s", bool fSpaces=true)
{
    printf(pszFormat, HeRETr(vch, fSpaces).c_str());
}
